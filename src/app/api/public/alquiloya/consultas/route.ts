...
export async function POST(request: Request) {
  try {
    ...
    // Para que tenga sentido, exigimos al menos algÃºn dato de contacto o el id de la propiedad.
    ...
    // Si no vino agente_id pero sÃ­ propiedad_id, intentamos resolver el agente de la propiedad.
    if (!agenteId && propiedadId) {
      try {
        const r = await queryWithRetry<{ agente_id: string | null }>(
          pool,
          `SELECT agente_id FROM "${SCHEMA}"."propiedades"
