...
export async function PATCH(request: Request, ctx: Ctx) {
  try {
    ...
    // aprobar â†’ crear agente o propietario y vincular resultado_id

    // Cliente admin para borrar auth.users huÃ©rfanos en la pre-validaciÃ³n y
    // para createUser dentro de la transacciÃ³n.
    const supabaseAdmin = createServiceRoleClient();

    // Pre-validar unicidad de email para agente/referido. Antes del check,
    // limpiamos filas HUÃ‰RFANAS de alquiloya.usuarios â€” son las que quedan
    ...
    if (sol.email && sol.tipo !== "propietario") {
      ...
      // auth_user_id para borrar tambiÃ©n en auth.users.
      ...
      for (const g of ghosts) {
        try {
          ...
          console.warn("[solicitudes-acceso] no se pudo borrar usuario huÃ©rfano:", e instanceof Error ? e.message : e);
        }
        if (g.auth_user_id) {
          try {
            await supabaseAdmin.auth.admin.deleteUser(g.auth_user_id);
          } catch (e) {
            console.warn("[solicitudes-acceso] no se pudo borrar auth.user huÃ©rfano:", e instanceof Error ? e.message : e);
          }
        }
      }

      // 2. Ahora sÃ­, chequeo de duplicados reales.
      ...
      if (dup.length > 0) {
        return NextResponse.json(
          {
            error:
              `El email "${sol.email}" ya estÃ¡ registrado en ${dup[0].source}. ` +
              `Pediles que usen otro correo o resolvÃ© el duplicado antes de aprobar.`,
          },
          { status: 409 }
        );
      }
    ...
    // todas las DBs de produccion â€” sin esto el INSERT mas abajo fallaba con
    ...
    // creado si la transacciÃ³n del ERP falla.
    let createdAuthUserId: string | null = null;
    try {
      await client.query("BEGIN");

      // Resolver plan tier â†’ uuid + billing para calcular vencimiento.
      ...
      if (sol.plan_tier_solicitado) {
        const pr = await client.query<{ id: string; billing: string | null }>(
          ...
          [EMPRESA_ID, sol.plan_tier_solicitado]
        );
      ...
      // partnerSlug solo aplica a referido_partner â€” lo necesitamos despues
      ...
      if (sol.tipo === "agente") {
        ...
        const r = await client.query<{ id: string }>(
          `INSERT INTO ${t("agentes")}
           ...
           RETURNING id`,
          [EMPRESA_ID, sol.nombre, sol.email, sol.telefono, cargo, planId]
        );
        ...
        const r = await client.query<{ id: string }>(
          `INSERT INTO ${t("propietarios")}
           ...
           RETURNING id`,
          [EMPRESA_ID, sol.nombre, sol.email, sol.telefono, planId]
        );
        resultadoId = r.rows[0].id;
      } else {
        ...
        const rl = await client.query<{ id: string }>(
          `INSERT INTO ${t("referral_links")}
           ...
           RETURNING id`,
          [EMPRESA_ID, resultadoId, partnerSlugRef, null]
        );
        await client.query(
          `INSERT INTO ${t("referral_commission_rules")}
           ...
           VALUES ($1::uuid, $2::uuid, $3::uuid, 'porcentaje', 10, NULL,
                   true, 12, now())`,
          [EMPRESA_ID, resultadoId, rl.rows[0].id]
        );
      }
      ...
      // registro en alquiloya.propietarios (ya creado arriba) â€” sin cuenta de
          ...
          throw new Error(
            isDup
              ? `El email "${sol.email}" ya tiene cuenta en Auth. ` +
                  `Borralo en Supabase Dashboard â†’ Authentication â†’ Users o usÃ¡ otro email.`
        ...
        createdAuthUserId = authUserId;
        {
          ...
          if (sol.tipo === "agente") {
            insertUsuario = await client.query<{ id: string }>(
              `INSERT INTO ${t("usuarios")}
               ...
               RETURNING id`,
              [EMPRESA_ID, authUserId, sol.email, sol.nombre, rol, resultadoId]
            );
          } else if (sol.tipo === "propietario") {
            insertUsuario = await client.query<{ id: string }>(
              `INSERT INTO ${t("usuarios")}
            ...
            insertUsuario = await client.query<{ id: string }>(
              `INSERT INTO ${t("usuarios")}
               ...
               RETURNING id`,
              [EMPRESA_ID, authUserId, sol.email, sol.nombre, rol]
            );
          }
          ...
          if (sol.tipo === "referido_partner" && usuarioErpId) {
            await client.query(
              ...
              [usuarioErpId, EMPRESA_ID, resultadoId]
            );
          }
          ...
          try {
            ...
            if (result.sent) {
              ...
              console.warn("[solicitudes-acceso] sendMail no enviÃ³:", result.reason);
            }
          } catch (e) {
            emailError = e instanceof Error ? e.message : "Error enviando mail";
            console.warn("[solicitudes-acceso] excepciÃ³n en sendMail:", emailError);
          }
        }
      }
      ...
      const upd = await client.query<{ id: string }>(
        `UPDATE ${t("solicitudes_acceso")}
          ...
          WHERE empresa_id=$1::uuid AND id=$2::uuid
          RETURNING id`,
        [EMPRESA_ID, id, resultadoId, user.id]
      );
      ...
      // Si llegamos a crear el auth.user pero la transacciÃ³n del ERP fallÃ³,
      // borramos el auth.user para no dejar cuentas huÃ©rfanas.
