...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const { rows } = await queryWithRetry<{ id: string }>(
    pool,
    `INSERT INTO "${SCHEMA}"."agente_tips" (empresa_id, agente_id, zona, titulo, body, orden, activo)
     VALUES ($1::uuid, $2::uuid, $3, $4, $5, $6, true) RETURNING id`,
    [EMPRESA_ID, id, zona, titulo, bodyTxt, Number.isFinite(orden) ? Math.trunc(orden) : 0]
  );
