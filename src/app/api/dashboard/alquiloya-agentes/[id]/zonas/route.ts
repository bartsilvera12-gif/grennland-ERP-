...
export async function GET(request: Request, ctx: Ctx) {
  ...
  const { rows } = await queryWithRetry(
    pool,
    `SELECT id, ciudad, barrio, orden FROM "${SCHEMA}"."agente_zonas"
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const { rows } = await queryWithRetry<{ id: string }>(
    pool,
    `INSERT INTO "${SCHEMA}"."agente_zonas" (empresa_id, agente_id, ciudad, barrio, orden)
     VALUES ($1::uuid, $2::uuid, $3, $4, $5) RETURNING id`,
    [EMPRESA_ID, id, ciudad, barrio, Number.isFinite(orden) ? Math.trunc(orden) : 0]
  );
