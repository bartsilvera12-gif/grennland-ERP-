      ...
      <div className="container" style={{ padding: '56px 32px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr 1fr', gap: 40 }}>
          ...
          <FootCol title="Contacto" items={(() => {
            ...
            const C = (typeof window !== 'undefined' && window.CONTACTO_ALQUILOYA) || {
              ...
              telefono: '0981 227 400',
              telefonoWa: '595981227400',
        ...
        <div style={{ borderTop: '1px solid #1b2a3a', marginTop: 40, paddingTop: 20, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div className="row gap-16" style={{ fontSize: 12.5, flexShrink: 0 }}>
            ...
            <a href={'https://wa.me/' + ((typeof window !== 'undefined' && window.CONTACTO_ALQUILOYA && window.CONTACTO_ALQUILOYA.telefonoWa) || '595981227400')} target="_blank" rel="noopener noreferrer" style={{ color: '#9aa4b1', textDecoration: 'none' }}>WhatsApp</a>
          </div>
        </div>
      </div>
    </footer>
...
// VIVIO inicializa.
const VIVIO_WA = (typeof window !== 'undefined' && window.CONTACTO_ALQUILOYA && window.CONTACTO_ALQUILOYA.whatsapp) || '595981227400';
...
// ────────────────────────────────────────────────────────────────────────────
// WhatsAppLauncher — boton flotante directo a WhatsApp (reemplazo de VIVIO).
// VIVIO queda definido (VivioChatbot arriba) pero NO se renderiza global; este
// componente es el que se monta en app.jsx. Decision P2B: el cliente prefiere
...
function WhatsAppLauncher() {
  ...
  return (
    <a
      ...
      title="Hablar con AlquiloYa"
      aria-label="Hablar con AlquiloYa por WhatsApp"
      style={{
        position: 'fixed', bottom: 24, right: 24, zIndex: 80,
        ...
        textDecoration: 'none',
        display: 'grid', placeItems: 'center',
        boxShadow: '0 10px 28px rgba(37,211,102,.45), 0 4px 10px rgba(37,211,102,.25)',
